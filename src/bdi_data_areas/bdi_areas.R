...
urls <- list(areas2021 = "BDI/2020-01-19/bdi_areas.geojson",
             districts2022 = "BDI/2022-03-30/Boundaries%20(3).geojson"
) %>%
...
# Matana split into Matana + Rutova
...
# Kinyinya split into Kinyinya + Gisuru
wide2022$area_id0[wide2022$area_name2 == "Gisuru"] <- "BDI"
wide2022$area_name0[wide2022$area_name2 == "Gisuru"] <- "Burundi"
wide2022$area_id1[wide2022$area_name2 == "Gisuru"] <- "BDI_1_18"
wide2022$area_id2[wide2022$area_name2 == "Gisuru"] <- "BDI_2_49lk"
wide2022$area_id[wide2022$area_name2 == "Gisuru"] <- "BDI_2_49lk"
wide2022$area_id[wide2022$area_name2 == "Kinyinya"] <- "BDI_2_46ib"
wide2022$area_id2[wide2022$area_name2 == "Kinyinya"] <- "BDI_2_46ib"



# Summary of changes 2021 -> 2022
a <- ggplot() +
  geom_sf(data = wide2022 %>% filter(area_id1 == "BDI_1_18"),
          color = "black", fill = NA) +
  geom_sf_text(data = wide2022 %>% filter(area_id1 == "BDI_1_18"),
               aes(label = area_name2, geometry = geometry)) +
  ggtitle("Ruyigi: district boundaries 2022")

b <- ggplot() +
  geom_sf(data = wide2021 %>% filter(area_id1 == "BDI_1_18"),
          color = "black", fill = NA) +
  geom_sf_text(data = wide2021 %>% filter(area_id1 == "BDI_1_18"),
               aes(label = area_name2, geometry = geometry)) +
  ggtitle("Ruyigi: district boundaries 2021")
...
p_2022_boundary_changes <- cowplot::plot_grid(a, b, c, d,  nrow = 2, ncol = 2, align = "hv")
...
plot(bdi_simple%>% filter(area_id1 == "BDI_1_18") %>% st_union())
